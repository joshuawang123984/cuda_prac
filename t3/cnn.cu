...
__global__ void softmax(float *x, int R, int C)
{
    ...
    for (int i = tid; i < C; i += blockDim.x)
    {
        float e = fexp(x[row * C + i] - max_val);
        x[row * C + i] = e;
        sum += e;
    }
...
{
    int out_row = blockDim.y *blockIdx.y + threadIdx.y int out_col = blockDim.x * blockIdx.x + threadIdx.x;
                       ...
                       int Cout, int K)
{
    ...
    __shared__ float tile[TILE + MAX_K][TILE + MAX_K];
...
__global__ conv_backward(float *X, float *dZ, float *dW, int N, int Cin, int H, int W, int Cout, int K)
{
    int cout = blockIdx.x;
    int cin = blockIdx.y;
    int i = threadIdx.x;
...
int main()
{
    ...
    dim3 block(16, 16);
    dim3 grid((C_out + 15) / 16, (R_out + 15) / 16);
    dim3 convgrid(
        (W_out + 15) / 16,
        (H_out + 15) / 16,
        N * Cout);
    ...
    conv2d<<<convgrid, block>>>();
    GeLU<<<...>>>();
    maxpool4d<<<...>>>();

    conv2d<<<...>>>();
    GeLU<<<...>>>();
    maxpool4d<<<grid, block>>>(input, output, R, C, 2, 2);
